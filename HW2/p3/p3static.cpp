...
//Program to verify the Goldbach Conjecture - OpenMP Static Scheduling
...
int main(int argc, char* argv[])
{
  ...
  for (int l = 0; l < N.size(); l++)
      ...
      for (int t = 0; t < 16; t++)
	{
	  ...
	  get_time(&before);
#pragma omp parallel for private(i) reduction(+:p) schedule(static, 10000)
	  for (i = 2; i <= N[l]/2; i++)
	    {
	      if (is_prime(i))
		{
